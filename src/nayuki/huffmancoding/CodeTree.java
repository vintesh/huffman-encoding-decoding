 ...
 */
/*
 * There are two main uses of a CodeTree: - Read the 'root' field and walk
 * through the tree to extract the desired information. - Call getCode() to get
 * the code for a symbol, provided that the symbol has a code.
 *
 * The path to a leaf node determines the leaf's symbol's code. Starting from
 * the root, going to the left child represents a 0, and going to the right
 * child represents a 1. Constraints: - The tree must be complete, i.e. every
 * leaf must have a symbol. - No symbol occurs in two leaves. - But not every
 * symbol needs to be in the tree. - The root must not be a leaf node. Example:
 * Huffman codes: 0: Symbol A 10: Symbol B 110: Symbol C 111: Symbol D Code
 * tree: . / \ A . / \ B . / \ C D
 */
